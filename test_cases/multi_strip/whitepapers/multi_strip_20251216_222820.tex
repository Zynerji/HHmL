...
Metric & Value \\
\midrule
Total Time & 256.0 s (4.27 min) \\
Avg Cycle Time & 0.512 s \\
...
The simulation tracked vortex formation and stability across 500 training cycles,
...
The RNN achieved a remarkable peak vortex density of \textbf{100.0%} at cycle 490,
...
& $\kappa$ (elongation) & 1.494 \\
& $\delta$ (triangularity) & 0.270 \\
& QEC layers & 5.1 \\
\midrule
...
\bottomrule
\end{tabular}
\caption{RNN parameter configuration at peak vortex density (cycle 490)}
\end{table}
...
\begin{enumerate}
    \item \textbf{Vortex Seeding Strategy}: Vortex seed strength of 0.48 indicates
    conservative vortex seeding,
    creating selective vortex formation.

    \item \textbf{Spectral vs Spatial Dynamics}: Spectral weight of 0.47 shows
    spatial-dominated propagation,
    utilizing local neighbor interactions.

    \item \textbf{Quality Control via Annihilation}:
    Active vortex pruning enabled
    (antivortex strength 0.49),
    with pruning threshold 0.54 targeting
    moderate to high-quality vortices.

    \item \textbf{Damping Balance}: Damping coefficient 0.112 provides
    strong energy dissipation,
    stabilizing vortex structures.

    \item \textbf{Nonlinear Interactions}: Nonlinearity parameter 0.155
    introduces vortex enhancement
...
\begin{itemize}
    \item Average vortex density: 55.1%
    \item High-density cycles ($>70\%$): 281 (28.1\%)
    \item Peak achieved at: 49.0\% through training
\end{itemize}
...
\begin{itemize}
    \item RNN-based discovery of parameter configurations that achieve 100.0% peak vortex density
    ...
    \item Scalable sparse graph representations for large-scale (4,000 node) systems
