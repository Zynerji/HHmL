...
class HolographicResonator:
    ...
    def initialize_sources_from_lattice(self, base_frequency: float = 1.0,
                                       ...
                                       use_helical_phase_weighting: bool = False,
                                       helical_omega: float = 0.3):
        """
        ...
        Args:
            ...
            use_helical_phase_weighting: If True, use logarithmic helical frequency spacing
                                        (adapted from Helical SAT Heuristic)
            helical_omega: Helical phase frequency parameter (default 0.3)
        """
        if seed is not None:
            np.random.seed(seed)
        ...
        # Helical phase weighting for frequencies (if enabled)
        if use_helical_phase_weighting:
            # Import here to avoid circular dependency
            try:
                import torch
                from ivhl.graph.spectral_optimizer import HelicalPhaseWeighting

                phase_weighting = HelicalPhaseWeighting(omega=helical_omega, device='cpu')
                frequencies_tensor = phase_weighting.compute_logarithmic_frequencies(
                    num_sources=self.num_sources,
                    base_freq=base_frequency,
                    freq_range=frequency_spread
                )
                frequencies = frequencies_tensor.numpy()
            except ImportError:
                # Fallback to uniform if torch not available
                frequencies = None
                print("Warning: torch not available, falling back to uniform frequencies")
        else:
            frequencies = None

        for i, pos in enumerate(lattice_points):
            # Frequency: helical phase weighting or uniform random
